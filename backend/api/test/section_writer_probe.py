...
from backend.config import DEFAULT_MODEL, OUTPUTS_DIR, load_prompt, load_mcp_server
from backend.domain import build_initial_context, persist_run_state, refresh_prompt_views, save_section_draft
from backend.domain.validation import aggregate_feedback
from backend.domain.visual_artifacts import materialize_visual_artifact, normalize_svg_markup
from backend.llm_io import parse_model
...
from backend.src import Agent, MCPManager, MCPServer
from backend.src.mcp_manager import result_to_content
...
if __name__ == "__main__":
    ...
    uvicorn.run("backend.api.test.section_writer_probe:app", host="127.0.0.1", port=8010, reload=False)
